...
import '../AstrasPage/AstrasPage.css';
// import { getSetData } from "../../services/gatcg-api";
// import { Card } from "../../models/card";


interface AstrasPageState {
    showing: boolean,
    // cards: Card[],
    cards: (any)[],
    imgSrc: string,
...
const initState: AstrasPageState = {
    showing: false,
    cards: [],
    imgSrc: "",
...
const AstrasPage: FC = () => {
    const [astrasState, setAstrasState] = useState(initState);
    const { id } = useParams();

    const handleTextToggle = () => {    
        const showing = astrasState.showing;
        setAstrasState({ ...astrasState, showing: !showing });
    };

    const showLightbox = (targetImage: string) => {
        setAstrasState({ ...astrasState, lightboxDisplay: true, cardToShow: targetImage });
    };
    
    const hideLightBox = () => {
        setAstrasState({ ...astrasState, lightboxDisplay: false });
    };
    ...
    <div>
        ...
        <div className="flexCardImgs">
            {astrasState.cards.length > 0 ?
                astrasState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${astrasState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
                        <p style={{ display: (astrasState.showing ? 'block' : 'none' ) }} id={card["collector_number"].toString()} className="centerText">{card.name}</p>
                        <br />
                    </div>
                ))
            : <p>No Cards!</p>}
        </div> 
        { astrasState.lightboxDisplay ? 
        <div id="lightbox" onClick={() => hideLightBox()}>
            <img id="lightbox-img"src={process.env.PUBLIC_URL + `/${astrasState.imgSrc}/${astrasState.cardToShow}.jpg`}/>
        </div> : '' }
    </div>
    </>;
} 

export default AstrasPage;