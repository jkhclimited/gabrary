...
import '../UmbralsPage/UmbralsPage.css';
// import { getSetData } from "../../services/gatcg-api";
// import { Card } from "../../models/card";


interface UmbralsPageState {
    showing: boolean,
    // cards: Card[],
    cards: (any)[],
    imgSrc: string,
...
const initState: UmbralsPageState = {
    showing: false,
    cards: [],
    imgSrc: "",
...
const UmbralsPage: FC = () => {
    const [umbralsState, setUmbralsState] = useState(initState);
    const { id } = useParams();

    const handleTextToggle = () => {    
        const showing = umbralsState.showing;
        setUmbralsState({ ...umbralsState, showing: !showing });
    };

    const showLightbox = (targetImage: string) => {
        setUmbralsState({ ...umbralsState, lightboxDisplay: true, cardToShow: targetImage });
    };
    
    const hideLightBox = () => {
        setUmbralsState({ ...umbralsState, lightboxDisplay: false });
    };

    useEffect(() => {
        if (id === "DOA-Alter") {
            const cardsList = DB_DOAAlter.filter((card: any) => card.types.includes('UMBRAL'));
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "DOAAlter_Images" });
        }
        if (id === "DOA1st") {
            const champsList = DB_DOA1st.filter((card: any) => card.types.includes('UMBRAL'));
            setUmbralsState({ ...umbralsState, cards: champsList, imgSrc: "DOA1st_Images"});
        }
        if (id === "DOAp") {
            const cardsList = DB_DOAp.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "DOAp_Images" });
        }
        if (id === "DOASD") {
            const cardsList = DB_DOASD.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "DOASD_Images" });
        }
        if (id === "EVP") {
            const cardsList = DB_EVP.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "EVP_Images" });
        }
        if (id === "FTC") {
            const cardsList = DB_FTC.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "FTC_Images" });
        }
        if (id === "FTCA") {
            const cardsList = DB_FTCA.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "FTCA_Images" });
        }
        if (id === "GVC") {
            const cardsList = DB_Isekai.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "GVC_Images" });
        }
        if (id === "KSP") {
            const cardsList = DB_KSP.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "KSP_Images" });
        }
        if (id === "P22") {
            const cardsList = DB_P22.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "P22_Images"});
        }
        if (id === "P23") {
            const cardsList = DB_P23.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "P23_Images"});
        }
        if (id === "SP1") {
            const cardsList = DB_SP1.filter((card: any) => card.types.includes('UMBRAL'));;
            setUmbralsState({ ...umbralsState, cards: cardsList, imgSrc: "SP1_Images"});
        }
        // const fixedID = id?.replace("-", " ");
        // getSetData(fixedID!!).then((cards) => {
        //     console.log(cards);
        //     const champsList = cards.filter((card: any) => card.types.includes('UMBRAL'));
    ...
    <div>
        ...
        <div className="flexCardImgs">
            {umbralsState.cards.length > 0 ?
                umbralsState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${umbralsState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
                        <p style={{ display: (umbralsState.showing ? 'block' : 'none' ) }} id={card["collector_number"].toString()} className="centerText">{card.name}</p>
                        <br />
                    </div>
                ))
            : <p>No Cards!</p>}
        </div> 
        { umbralsState.lightboxDisplay ? 
        <div id="lightbox" onClick={() => hideLightBox()}>
            <img id="lightbox-img"src={process.env.PUBLIC_URL + `/${umbralsState.imgSrc}/${umbralsState.cardToShow}.jpg`}/>
        </div> : '' }
    </div>
    </>;
} 

export default UmbralsPage;