...
const WindsPage: FC = () => {
    ...
    const showLightbox = (targetImage: string) => {
        setwindsState({ ...windsState, lightboxDisplay: true, cardToShow: targetImage });
    };
    
    const hideLightBox = () => {
        setwindsState({ ...windsState, lightboxDisplay: false });
    };
    ...
    useEffect(() => {
        ...
        if (id === "FTC") {
            const cardsList = DB_FTC.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "FTC_Images" });
        }
        if (id === "FTCA") {
            const cardsList = DB_FTCA.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "FTCA_Images" });
        }
        if (id === "GVC") {
        ...
        if (id === "P23") {
            const cardsList = DB_P23.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "P23_Images"});
        }
        if (id === "P24") {
            const cardsList = DB_P24.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "P24_Images"});
        }
        if (id === "ALC") {
            const cardsList = DB_ALC.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "ALC_Images"});
        }
        if (id === "SP1") {
        ...
        if (id === "spoilers") {
            const cardsList = DB_Spoilers.filter((card: any) => card.element.includes('WIND'));;
            setwindsState({ ...windsState, cards: cardsList, imgSrc: "Spoilers_Images"});
        }
    }, [])
            ...
            {windsState.cards.length > 0 ?
                windsState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${windsState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
