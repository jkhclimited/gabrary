            ...
            {arcanesState.cards.length > 0 ?
                arcanesState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${arcanesState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
