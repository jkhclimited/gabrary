...
const ArcanesPage: FC = () => {
    ...
    useEffect(() => {
        ...
        if (id == "DOA1st") {
            const champsList = DB_DOA1st.filter((card: any) => card.element.includes('ARCANE'));
            setarcanesState({ ...arcanesState, cards: champsList, imgSrc: "DOA1st_Images"});
        }
        if (id == "DOAp") {
            const cardsList = DB_DOAp.filter((card: any) => card.element.includes('ARCANE'));;
    ...
    <div>
            ...
            {arcanesState.cards.length > 0 ?
                arcanesState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${arcanesState.imgSrc}/${parseInt(card.collector_number)}.jpg`} alt="" /></p>
        ...
        <div id="lightbox" onClick={() => hideLightBox()}>
            <img id="lightbox-img"src={process.env.PUBLIC_URL + `/${arcanesState.imgSrc}/${arcanesState.cardToShow}.jpg`}/>
        </div> : '' }
    </div>
    </>;
} 
