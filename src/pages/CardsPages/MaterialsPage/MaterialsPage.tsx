...
const MaterialsPage: FC = () => {
    ...
    const showLightbox = (targetImage: string) => {
        setMatState({ ...matState, lightboxDisplay: true, cardToShow: targetImage });
    };
    
    const hideLightBox = () => {
        setMatState({ ...matState, lightboxDisplay: false });
    };

    useEffect(() => {
        if (id == "DOA-Alter") {
            const cardsList = DB_DOAAlter.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "DOAAlter_Images" });
        }
        if (id == "DOA1st") {
            const champsList = DB_DOA1st.filter((card: any) => card.types.includes('REGALIA'));
            setMatState({ ...matState, cards: champsList, imgSrc: "DOA1st_Images"});
        }
        if (id == "DOAp") {
            const cardsList = DB_DOAp.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "DOAp_Images" });
        }
        if (id == "DOASD") {
            const cardsList = DB_DOASD.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "DOASD_Images" });
        }
        if (id == "EVP") {
            const cardsList = DB_EVP.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "EVP_Images" });
        }
        if (id === "FTC") {
            const cardsList = DB_FTC.filter((card: any) => card.types.includes('REGALIA'));;;
            setMatState({ ...matState, cards: cardsList, imgSrc: "FTC_Images" });
        }
        if (id === "FTCA") {
            const cardsList = DB_FTCA.filter((card: any) => card.types.includes('REGALIA'));;;
            setMatState({ ...matState, cards: cardsList, imgSrc: "FTCA_Images" });
        }
        if (id == "GVC") {
            const cardsList = DB_Isekai.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "GVC_Images" });
        }
        if (id == "KSP") {
            const cardsList = DB_KSP.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "KSP_Images" });
        }
        if (id == "P22") {
        ...
        if (id == "P24") {
            const cardsList = DB_P24.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "P24_Images"});
        }
        if (id == "ALC") {
            const cardsList = DB_ALC.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "ALC_Images"});
        }
        if (id == "SP1") {
        ...
        if (id == "spoilers") {
            const cardsList = DB_Spoilers.filter((card: any) => card.types.includes('REGALIA'));;
            setMatState({ ...matState, cards: cardsList, imgSrc: "Spoilers_Images"});
        }
    }, [])
