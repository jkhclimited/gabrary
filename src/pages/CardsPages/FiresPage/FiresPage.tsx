...
const FiresPage: FC = () => {
    ...
    useEffect(() => {
        ...
        if (id == "P23") {
            const cardsList = DB_P23.filter((card: any) => card.element.includes('FIRE'));;
            setfireState({ ...fireState, cards: cardsList, imgSrc: "P23_Images"});
        }
        if (id == "P24") {
            const cardsList = DB_P24.filter((card: any) => card.element.includes('FIRE'));;
            setfireState({ ...fireState, cards: cardsList, imgSrc: "P24_Images"});
        }
        if (id == "ALC") {
            const cardsList = DB_ALC.filter((card: any) => card.element.includes('FIRE'));;
            setfireState({ ...fireState, cards: cardsList, imgSrc: "ALC_Images"});
        }
        if (id == "SP1") {
        ...
        if (id == "spoilers") {
            const cardsList = DB_Spoilers.filter((card: any) => card.element.includes('FIRE'));;
            setfireState({ ...fireState, cards: cardsList, imgSrc: "Spoilers_Images"});
        }
    }, [])
    ...
    <div>
            ...
            {fireState.cards.length > 0 ?
                fireState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${fireState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
        ...
        <div id="lightbox" onClick={() => hideLightBox()}>
            <img id="lightbox-img"src={process.env.PUBLIC_URL + `/${fireState.imgSrc}/${fireState.cardToShow}.jpg`}/>
        </div> : '' }
    </div>
    </>;
} 
