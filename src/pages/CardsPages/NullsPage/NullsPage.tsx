...
const NullsPage: FC = () => {
    ...
    useEffect(() => {
        ...
        if (id == "P23") {
            const cardsList = DB_P23.filter((card: any) => card.element.includes('NORM'));;
            setnullsState({ ...nullsState, cards: cardsList, imgSrc: "P23_Images"});
        }
        if (id == "P24") {
            const cardsList = DB_P24.filter((card: any) => card.element.includes('NORM'));;
            setnullsState({ ...nullsState, cards: cardsList, imgSrc: "P24_Images"});
        }
        if (id == "ALC") {
            const cardsList = DB_ALC.filter((card: any) => card.element.includes('NORM'));;
            setnullsState({ ...nullsState, cards: cardsList, imgSrc: "ALC_Images"});
        }
        if (id == "SP1") {
        ...
        if (id == "spoilers") {
            const cardsList = DB_Spoilers.filter((card: any) => card.element.includes('NORM'));;
            setnullsState({ ...nullsState, cards: cardsList, imgSrc: "Spoilers_Images"});
        }
    }, [])
            ...
            {nullsState.cards.length > 0 ?
                nullsState.cards.map(card => (
                    <div className="text-row" key={card["name"].toString()}>                           
                        <p className="cardImgBox" id={card["name"].toString()}><img className="cardImg" onClick={() => showLightbox(card.collector_number)} src={process.env.PUBLIC_URL + `/${nullsState.imgSrc}/${card.collector_number}.jpg`} alt="" /></p>
