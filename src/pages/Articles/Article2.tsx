...
const Article2: FC = () => {
    ...
    <div className='indivArticle-article'>
        ...
        If you have not joined the discord community come <a target="_blank" rel="noopener noreferrer" href="https://discord.gg/grandarchivetcg">here</a><br/><br/>
        Check out the top 8 results below as well as some comments from the two 5-0 players!<br/><br/>
        5-0 Player #1: AzNat - <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@AzNat/CxnqGjK2GxwH280oJbtU">Decklist</a><br/><br/>
        ...
        Fire Lorraine’s <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/135")}>Flame Sweep</b> in combination with her level 2s on enter effect that buffs her attacks for the turn by 2 and draws a card whenever she kills a unit. In general they will play around a potential <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/213")}>Favorable Wind</b>s that would buff any <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/216")}>Phalanx Captain</b>s and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/226")}>Tactful Sergeant</b>s to 5 health. With so many fire Lorraine’s running rampart in events at the moment I knew I needed a way to somehow off-set these huge swing turns their lvl 2 could bring. Which lead me to the odd card out of this deck compared to other wind Lorraine builds I’ve seen, <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/110")}>Song of Nurturing</b>, for 2 costs provides all allies with 2 extra health for a turn.<br/><br/>
        I included this at 4 in this deck in an attempt to stop my early game <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/226")}>Tactful Sergeant</b>s and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/216")}>Phalanx Captain</b>s from being turned into a +1 in card advantage for my opponent by a <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/135")}>Flame Sweep</b> (+2 if you count the now dead ally). The song wont save any allies that don’t already have 4 health, eg <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/225")}>Swift Recruit</b> and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/91")}>Honorable Vanguard</b>, but those allies are often preferred to be in discard pile anyways to be used in following turns as floating memory.<br/><br/>
        ...
        5-0 Player #2: Blueskycandy - <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@BlueSkyCandy/vPxrLXr1PgHmgGtYBz9d">Decklist</a><br/><br/>
        This deck was selected because it is quite an all-rounded deck with multiple angles to close the game. The basis of this deck construction uses the shell of the famous “Ferrari Wind Lorraine” deck made by limelight that allows you to draw lots of cards when you hit level 3. The ability to draw lots of cards allows you to play different lines, as well as counteract the loss of cards when you are blitzing to level 3 in various ways (e.g <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/201")}>Beseech the Winds</b> and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/81")}>Dungeon Guide</b>s). The 2 main ways to close the game are the wind human tribal package with lots of units, and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/253")}>Spirit Blade: Ensoul</b> in combination with <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/71")}>Banner Knight</b> (if you have enough cards for it) which I took inspiration from BillytheKid/Scrambyeggs after playing against him in a local tournament in Australia.<br/><br/>
        In terms of sideboard, 2 <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/207")}>Cry for Help</b> are useful against the traditional “Ferrari Wind Lorraine” or the Fire variant of it that utilizes <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/255")}>Spirit Blade: Infusion</b> to buff a sword massively to swing at the champion. The extra phalanx captain as well is useful against decks that do poorly against large board states like Rai or possibly Zander pre-luxem. <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/252")}>Spirit Blade: Dispersion</b> is useful against water decks or Rai where they run defenses like <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/197")}>Water Barrier</b> and <b className="embedIMG" onClick={() => showLightbox("/DOAAlter_Images/247")}>Spellshield: Arcane</b>.<br/><br/>
        ...
        3rd: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@--VIjgHrA0SX03Bw8JF31IA4zH148KKiEcNCiVFILaFZm42BA1A+cio9W4AFJkIpYQA6YkEHzcET8D6zdzDFTMFgIAgAAvkAA">CAWFEEtheMerchant</a><br/>
        4th: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@Tan/SAC0D2gh1XuIV0PhDGTQ">t.ang</a><br/>
        5th: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@Mazurani/66mHixIO6luBpPYY9rUW">Mazurani 3780</a><br/>
        6th: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@JimmyLe/JwVYyMJWXlhi8MrPcNQl">Jimmyhl1329</a><br/>
        7th: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@Biefall/bQQUyBbbA5VRBx6FKz6Z">Biefall</a><br/>
        8th: <a target="_blank" rel="noopener noreferrer" href="https://build.silvie.org/@Anemone/FdNN7jM79D8kJ4Jx2lB7">Anemone</a><br/><br/><br/>
    </div>
